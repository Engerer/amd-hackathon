...
def main():
    ...
    with TemporaryDirectory() as tmp:
        ...
        # Review pass now runs for all categories: each task makes an initial call +
        # one review call = 2 * len(tasks) total. Allow a small tolerance in case any
        # review was skipped due to deadline constraints during the run.
        min_expected_calls = len(tasks)          # at least 1 call per task
        max_expected_calls = len(tasks) * 2      # at most 2 calls per task (initial + review)
        if not (min_expected_calls <= len(CALLS) <= max_expected_calls):
            raise SystemExit(
                f"expected between {min_expected_calls} and {max_expected_calls} Fireworks calls, "
                f"got {len(CALLS)}"
            )
